...
const getPdfUploadPage = async (req, res) => {
  try {
...
const postPdfUpload = async (req, res) => {
  let document = null;
  try {
    if (!req.file) {
      const documents = await Document.find({ userId: req.session.user.id }).sort({ createdAt: -1 }).lean();
...
const getPdfList = async (req, res) => {
  try {
    ...
    console.error(err);
    res.status(500).send("Error loading PDF list");
  }
};

const getPdfQuestionPage = async (req, res) => {
  try {
    ...
    const history = await PdfQuestion.find({ documentId: document._id, userId: req.session.user.id })
      .sort({ askedAt: -1 })
      .lean();
    res.render("pdf-question", {
    ...
    console.error(err);
    res.status(500).send("Error loading question page");
  }
};

const postPdfQuestion = async (req, res) => {
  try {
    ...
    const topChunks = await searchChunks({
      userId: req.session.user.id,
      documentId: document._id,
      queryEmbedding,
      topK: 5,
    ...
    await PdfQuestion.create({
      ...
      askedAt: new Date(),
      confidence:
        topChunks.reduce((sum, chunk) => sum + (chunk.score || 0), 0) / Math.max(1, topChunks.length),
    });
    ...
    const message =
      status === "failed"
        ? "Could not answer the question. Your question has been saved, try again later."
        : null;
    res.render("pdf-question", {
      user: req.session.user,
      document: document.toObject(),
    ...
    console.error(err);
    const document = await Document.findOne({ _id: req.params.id, userId: req.session.user.id }).lean();
    const history = await PdfQuestion.find({ documentId: req.params.id, userId: req.session.user.id })
      .sort({ askedAt: -1 })
      .lean();
    res.render("pdf-question", {
      user: req.session.user,
      ...
      answer: null,
      question: "",
      message: "Could not answer the question. Please try again.",
    });
  }
};
